...
- **Environment**: `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY`, `SUPABASE_SERVICE_ROLE_KEY` (server-only, for migration tooling / webhook handlers landing in later changes). Google OAuth client ID/secret configured inside the Supabase dashboard, not in `.env`.
